...
@section('content')
    ...
    @component('admin.components.card')
        ...
        @slot('card_body')
            ...
            {!! Form::open(array('route' => 'milestones.store','method'=>'POST')) !!}
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        {!! Form::checkbox('is_a_priority', null, false)!!}
                    </div>
                </div>
                            ...
                            @foreach($parentCategories as $category)
                                ...
                                @if(count($category->subcategories))
                                    @foreach($category->subcategories as $subcategory )
                                        <option value="{{ $subcategory->number }}">{{ $subcategory->number }} {{ $subcategory->name }}</option>
                                    @endforeach
                                @endif
                                </optgroup>
                            @endforeach
